GenITRSplit <- function(data = list(predictor, treatment, outcome),
    ...
    while (diff > 10 ^ -3 & iter < 100) {
      ...
      diff <-
        max(abs(fit_coef$par / sqrt(sum(fit_coef$par ^ 2)) - betaPast))
      iter <- iter + 1
    }
    ...
    for (i in 1:sampleSize) {
      ...
      for (j in 1:sampleSize) {
        if (i != j) {
          hopt <-
            sqrt(sum((
              data$predictor[sampleSplitIndex, ][i, ] - data$predictor[sampleSplitIndex, ][j, ]
            ) ^ 2)) / sqrt(sampleSize)
          weight_v <-
            -pair_diff[i, j] * exp(-0.5 * link_diff[i, j] ^ 2 / (hopt ^ 2)) * link_diff[i, j] /
            sqrt(2 * pi)
          weight_delta <-
            pair_diff[i, j] * exp(-0.5 * link_diff[i, j] ^ 2 / (hopt ^ 2)) / sqrt(2 *
                                                                                    pi)
          V <-
            V + weight_v * (data$predictor[sampleSplitIndex, ][i, ] - data$predictor[sampleSplitIndex, ][j, ]) %*% t(data$predictor[sampleSplitIndex, ][i, ] -
                                                                                                                       data$predictor[sampleSplitIndex, ][j, ]) / hopt ^ 2
          Deltatmp <-
            Deltatmp + weight_delta * (data$predictor[sampleSplitIndex, ][i, ] - data$predictor[sampleSplitIndex, ][j, ]) / hopt
        }
      }
